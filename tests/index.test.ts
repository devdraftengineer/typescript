...
import { APIPromise } from 'devdraft/core/api-promise';

import util from 'node:util';
import Devdraft from 'devdraft';
import { APIUserAbortError } from 'devdraft';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['DEVDRAFT_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: Devdraft) => {
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['DEVDRAFT_LOG'] = 'debug';
      const client = new Devdraft({
        logger: logger,
        apiKey: 'My API Key',
        secret: 'My Secret',
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['DEVDRAFT_LOG'] = 'not a log level';
      const client = new Devdraft({
        logger: logger,
        apiKey: 'My API Key',
        secret: 'My Secret',
      ...
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'DEVDRAFT_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['DEVDRAFT_BASE_URL'] = undefined;
    });
    ...
    test('env variable', () => {
      process.env['DEVDRAFT_BASE_URL'] = 'https://example.com/from_env';
      const client = new Devdraft({
        apiKey: 'My API Key',
        secret: 'My Secret',
        idempotencyKey: 'My Idempotency Key',
    ...
    test('empty env variable', () => {
      process.env['DEVDRAFT_BASE_URL'] = ''; // empty
      const client = new Devdraft({
        apiKey: 'My API Key',
        secret: 'My Secret',
        idempotencyKey: 'My Idempotency Key',
      });
      expect(client.baseURL).toEqual('https://api.devdraft.ai');
    });

    test('blank env variable', () => {
      process.env['DEVDRAFT_BASE_URL'] = '  '; // blank
      const client = new Devdraft({
        apiKey: 'My API Key',
        secret: 'My Secret',
        idempotencyKey: 'My Idempotency Key',
    ...
    test('in request options overridden by env variable', () => {
      process.env['DEVDRAFT_BASE_URL'] = 'http://localhost:5000/env';
      const client = new Devdraft({
        apiKey: 'My API Key',
        secret: 'My Secret',
        idempotencyKey: 'My Idempotency Key',
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['DEVDRAFT_API_KEY'] = 'My API Key';
    process.env['DEVDRAFT_SECRET'] = 'My Secret';
    process.env['DEVDRAFT_IDEMPOTENCY_KEY'] = 'My Idempotency Key';
    const client = new Devdraft();
    expect(client.apiKey).toBe('My API Key');
    expect(client.secret).toBe('My Secret');
    expect(client.idempotencyKey).toBe('My Idempotency Key');
  });

  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['DEVDRAFT_API_KEY'] = 'another My API Key';
    process.env['DEVDRAFT_SECRET'] = 'another My Secret';
    process.env['DEVDRAFT_IDEMPOTENCY_KEY'] = 'another My Idempotency Key';
    const client = new Devdraft({
      apiKey: 'My API Key',
      secret: 'My Secret',
      idempotencyKey: 'My Idempotency Key',
